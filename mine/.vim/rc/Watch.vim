...
function! s:CoffeeCompileToBuf(file, buf, startline, endline)
  let src = bufnr('%')
  ...
  " \                   ' ' . b:coffee_litcoffee .
  " \                   ' 2>&1', input)
  let output = readfile(a:file, "binary")
...
function! s:CoffeeWatchUpdate()
  call s:CoffeeCompileToBuf(b:out_file, b:coffee_watch_buf, 1, '$')
...
\        Watch call s:CoffeeWatch(<q-args>)
